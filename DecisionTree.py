...
def find_best_split(rows, header):
    ...
    #print('Current uncertainty : ' + str(current_uncertainty));
...
## TODO: Step 2
def extract_major_Class(predictions):
    major_class_label = ''
    major_class_count = 0
    for class_label, count in predictions.iteritems():
        if count > major_class_count:
            major_class_label = class_label
    return major_class_label
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.id = id
        self.depth = depth
        self.predicted_class_label = extract_major_Class(class_counts(rows)) 
...
class Decision_Node:
    ...
    def __init__(self,
                 ...
                 rows,
                 pruned=0):
        #print ("question : " + str(question.column))
        #print ("rows : " + str(rows))
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print (colored(spacing + "Predicted Major Class : " + node.predicted_class_label, 'green'))
        return

    # Print the question at this node
    print (spacing + str(node.question))
    print (colored(spacing + "Decision node ID: " + str(node.id), 'green'))
    print (colored(spacing + "Decision node Depth: " + str(node.depth), 'green'))
    # Call this function recursively on the true branch
    print (spacing + '--> True:')
...
def computeAccuracy(rows, node):
    ...
    for row in rows:
        ...
        #print (predicted_label, row[-1])
        if predicted_label == row[-1]:
            numAccurate = numAccurate + 1        
    print(numAccurate)
    print (totalRows)
    print(numAccurate/totalRows)
